...
{
    let listener = lrcall::serde_transport::tcp::listen("localhost:0", Json::default)
        .await?
    ...
    Resp: Send + Sync + 'static,
{
    let stub = load_balance::RoundRobin::new(
        backends
            .into_iter()
            .map(|transport| lrcall::client::new(client::Config::default(), transport).spawn())
            .collect(),
    );
...
async fn main() -> anyhow::Result<()> {
    ...
    let double_listener = lrcall::serde_transport::tcp::listen("localhost:0", Json::default)
        .await?
        .filter_map(|r| future::ready(r.ok()));
