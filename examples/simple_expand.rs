...
pub trait World: ::core::marker::Sized + ::core::clone::Clone + Send + 'static {
    ...
    async fn logimesh_tcp_listen<A>(self: Pin<&mut Self>, config: ::logimesh::server::TcpConfig<A>) -> anyhow::Result<()>
    where
        A: ::tokio::net::ToSocketAddrs,
    {
        use ::logimesh::server::incoming::Incoming;
        let serve: ServeWorld<Self> = self.logimesh_serve();
        // let serve = self.logimesh_serve();
        let mut listener = ::logimesh::transport::tcp::listen(
            config.listen_address(),
            config.transport_codec().to_fn::<::logimesh::ClientMessage<WorldRequest>, ::logimesh::Response<WorldResponse>>(),
        )
        .await?;
        ...
        listener
            ...
            // the generated Component trait.
            .map(|channel| {
                channel.execute(self.logimesh_serve()).for_each(|fut| async {
